// 分页
	var handlePagination = function(pageNum) {
        
        var pIndex = GetQueryString("pIndex");
        
        if (pIndex == null){
        	
        	pIndex = 1;
        
        }
            
        $('#pager').bootpag({
        	
            total: pageNum,
            
            page: pIndex,
            
            maxVisible: 6 
            
            
        }).on('page', function(event, num){
        	
        	var parameters = getQueryParameters(num);
        	var userId = $("#userId").val();
            window.location.href='/user/userBankCard?' + parameters + '&userId='+userId;
            
        });

    }
	
	var GetQueryString = function(name){
		
        var reg = new RegExp('(^|&)'+ name +'=([^&]*)(&|$)');
        
        var r = window.location.search.substr(1).match(reg);
        
        if(r!=null) return decodeURIComponent(r[2]); return null;
   
	}
	
	var getQueryParameters = function(pIndex){
		
		var bankCode = GetQueryString('bankCode');// 银行编码
    	
        var bankAccount = GetQueryString('bankAccount');// 银行账号
        
        if (pIndex == null || pIndex == "") {
        	
        	pIndex = 1;
			
		}
        
        var paraStr = "pIndex=" + pIndex;
        
        if (bankCode != null && bankCode != ""){
        	
            paraStr += "&bankCode=" + bankCode;
        
        }
        
        if (bankAccount != null && bankAccount != ""){
        	
            paraStr += "&bankAccount=" + bankAccount;
        
        }
        
        return paraStr;
		
	}